...
var TWeb = {
  ...
  highlightPost: function(post_id, scroll) {
    ...
    if (scroll) {
      $postWrap.scrollIntoView({position: 'center', padding: 5});
    }
    ...
    var _load = function(url, before, after) {
      $.ajax(url, {
        ...
        success: function(data) {
          ...
          if (before) {
            var firstWrapEl = $moreEl.next('.js-widget_message_wrap').get();
            ...
            $data.insertBefore($moreEl);
            var st = $moreEl.offset().top - y;
            $moreEl.remove();
            $(window).scrollTop(st);
          } else {
            var lastWrapEl = $moreEl.prev('.js-widget_message_wrap').get();
        ...
        error: function(data) {
          var timeout = $moreEl.data('timeout') || 1;
          $moreEl.data('timeout', timeout * 2);
